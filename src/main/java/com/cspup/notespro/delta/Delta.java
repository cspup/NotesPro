...
import static com.cspup.notespro.delta.Op.Type.DELETE;
import static com.cspup.notespro.delta.Op.Type.INSERT;
...
public class Delta {
    ...
    /**
     * 操作转换
     * @param otherDelta 其它操作
     * @param priority 当前操作是否优先
     * @return 转换后的操作
     */
    public Delta transform(Delta otherDelta, boolean priority) {
        OpList.Iterator thisIterator = this.ops.iterator();
        OpList.Iterator otherIterator = otherDelta.ops.iterator();
        Delta delta = new Delta();
        while (thisIterator.hasNext() || otherIterator.hasNext()) {
            // A是插入操作，B不是插入操作，保留A的操作
            if (thisIterator.peekType()==INSERT&&(priority||otherIterator.peekType()!=INSERT)){
                Op op = thisIterator.next();
                Integer len = op.length();
                delta.retain(len);
            }else if (otherIterator.peekType()==INSERT){
                delta.push(otherIterator.next());
            }else {
                Integer length = Math.min(thisIterator.peekLength(), otherIterator.peekLength());
                Op thisOp = thisIterator.next(length);
                Op otherOp = otherIterator.next(length);
                if (thisOp.isDelete()){
                    // delete either makes their delete redundant or removes their retain
                    continue;
